...
import {  updateEventInDb } from "@/api/api";
...
const formSchema = z.object({
  ...
  image: z.string().optional(), // Image URL after upload
});

const EditCampaign = () => {
  const { user } = useContext(AuthContext);
  const { state } = useLocation();
  const { title, description, target, deadline, image, category, pid } = state;
  ...
  const handleUpload = async () => {
    ...
    formData.append("upload_preset", "funding"); // Replace with your unsigned preset
    formData.append("cloud_name", "defswxqkw"); // Replace with your Cloudinary cloud name
    ...
    try {
      ...
      setImageUrl(response.data.secure_url); // Set uploaded image URL
      form.setValue("image", response.data.secure_url); // Update form value
  ...
  const onSubmit = async (values) => {
    setIsLoading(true);
    console.log("Form Values:", { ...values, image: imageUrl, user });
    try {
  ...
  return (
    ...
    <div className="flex justify-center items-center flex-col rounded-[10px] sm:p-10 p-4">
      <div className="mx-auto max-w-3xl text-center mb-14">
        <h1 className="mt-5 text-3xl font-light leading-tight text-white sm:text-5xl sm:leading-tight lg:text-6xl lg:leading-tight">
          <span className="relative inline-flex justify-center whitespace-nowrap font-bold">
            <svg
              className="absolute -bottom-8 hidden w-5/6 text-orange-600 sm:block"
              viewBox="0 0 490 42"
              fill="none"
      ...
      <Form {...form}>
        ...
        >
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="text-white">
                ...
                <FormControl>
                  <Input
                    placeholder="What is your Campaign Category?"
                    {...field}
                  />
                </FormControl>
                <FormDescription>
                  Your Campaign Comes under which Category [Ex: Education,
                  Health etc]
                </FormDescription>
                <FormMessage />
              </FormItem>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="text-white">
                ...
                <FormControl>
                  <Textarea
                    placeholder="Tell why this Campaign is important to you."
                    {...field}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="text-white">
                ...
                <FormControl>
                  <Input
                    placeholder="Enter the target amount in USD"
                    {...field}
                    type="number"
                  />
                </FormControl>
                <FormDescription>
                  Specify the amount you want to raise for this campaign.
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="text-white">
                ...
                <FormDescription>
                  Select the deadline for the campaign.
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
          ...
          <div className="text-white">
            <label className="block mb-2">Upload Campaign Image</label>
            ...
            <FormDescription>
              Upload the Image before you submit the form.
            </FormDescription>
            {imageUrl && (
              <div className="mt-4">
                <img
                  src={imageUrl}
                  alt="Uploaded"
                  className="h-40 object-contain rounded mx-auto"
                />
              </div>
            )}
          </div>
      ...
      <Button  variant="ghost"  className="w-full bg-red-600 text-white">
            Cancel
          </Button>
