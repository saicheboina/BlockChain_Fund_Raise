...
export const columns = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="capitalize">{dayjs(row.getValue("date")).fromNow()}</div>
    ),
    enableSorting: true,
  },
  {
      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <Link to={`/campaign-details/${payment.pid}`}>
            ...
            <DropdownMenuItem
              onClick={() => {
                window.open("https://forms.gle/BndEM825vg9fV3FU7", "_blank");
              }}
...
const exportToCSV = (data, filename) => {
  ...
  // Map each object into a row of CSV values
...
function DataTableDemo() {
  const { user, allTransactions: transactions,allCampaigns } = React.useContext(AuthContext);


  const updatedTransactions = React.useMemo(() => {
    return transactions.map((trans) => {
      const theCamp = allCampaigns.find((camp) => camp.pid === trans.pid);
      return {
        ...trans,
        title: theCamp ? theCamp.title : "",
      };
    });
  }, [transactions, allCampaigns]);

  const allTransactions = React.useMemo(() => {
    if (user?.role === "Admin") {
      return updatedTransactions;
    } else if (user?.role === "Investor") {
      return updatedTransactions.filter((trans) => trans.uid === user.uid);
    } else {
      // Filter for company role
      const userCampaignPids = allCampaigns
        .filter((campaign) => campaign.uid === user.uid)
        .map((campaign) => campaign.pid);
  
      return updatedTransactions.filter((transaction) =>
        userCampaignPids.includes(transaction.pid)
      );
    }
  }, [user, updatedTransactions, allCampaigns]);
  ...
  return (
    <>
      ...
      <div className="w-full text-white sm:p-10 p-4 mt-4">
        <div className="mx-auto max-w-3xl text-center mb-14">
          <h1 className="mt-5 text-3xl font-light leading-tight text-white sm:text-5xl sm:leading-tight lg:text-6xl lg:leading-tight">
            <span className="relative inline-flex justify-center whitespace-nowrap font-bold">
              ...
              {user?.role === "Admin" ? "All" : "My"} {user?.role === "Company" ? "Funds" : "Transactions"}
            </span>
          </h1>
        </div>
        <div className="flex items-center py-4">
          ...
          <Button
            ...
            className="ml-8"
            onClick={() => {
              exportToCSV(allTransactions,"Transactions");
            }}
          >
            Export <Share />
          </Button>
